...
def compatibles(spectacle1, spectacle2):
    ...
    return not (spectacle1["debut"] < spectacle2["fin"] and spectacle2["debut"] < spectacle1["fin"])
...
def tous_compatibles(selection, spectacle):
    ...
    for spec in selection:
        if not compatibles(spectacle, spec):
            return False
    return True
...
def tri_selon_duree(programme):
    """ trie les spectacles du programme selon leur heure de début """
    def duree_spec(spectacle):
        return spectacle["fin"] - spectacle["debut"]
    return sorted(programme, key=duree_spec)

# TRI SELON HEURE DE FIN

def tri_selon_fin(programme):
    """ trie les spectacles du programme selon leur heure de début """
    def heure_fin(spectacle):
        return spectacle["fin"]
    return sorted(programme, key= heure_fin)
...
def selection1(programme):
    ...
    """
    prog = tri_selon_debut(programme)
    spec = prog[0]
    res = []
    while prochain_spectacle1(prog, spec["fin"]) is not None:
        res.append(spec)
        spec = prochain_spectacle1(prog, spec["fin"])
    res.append(spec)
    return res
...
def prochain_spectacle(programme, selection):
    ...
    for spec in programme:
        if tous_compatibles(selection, spec):
            return spec
    return None
...
def selection2(programme):
    ...
    """
    prog = tri_selon_duree(programme)
    print(prog)
    res = []
    for spec in prog:
        print(res)
        if prochain_spectacle(spec, res) is not None:
            res.append(spec)
    return res
