...
def afficher_menu(titre="Met un titre !", liste_options=["Met au moins une option !"]):
    ...
    for option in range(len(liste_options)):
        print(" \033[1;"+str(couleur)+"m"+str(option+1)+" -> "+str(liste_options[option])+ '\033[0m')
        couleur += 1
        if couleur == 36:
            couleur = 31
...
def programme_principal():
    ...
    while True:
        ...
        if rep == 1:
            ...
            while non_réussi:
                try:
                    liste_matchs = histoire2foot.charger_matchs("./"+input("Quel fichier charger ? \n"))
                    print(str(len(liste_matchs))+" matchs trouvées !")
                    non_réussi = False
                except:
                    quitter = input("Ce fichier n'a pas pu être trouver. Veux tu quitter ? (o pour quitter sinon entrée)\n" )
                    if quitter == "o":
                        non_réussi = False
        # Option 2
        if rep == 2:
            if liste_matchs == None:
                ...
                while non_réussi:
                    ...
                    matchs_ville = histoire2foot.matchs_ville(liste_matchs, str(ville))
                    if matchs_ville == []:
                        print("Aucun match ne c'est déroulé dans cette ville\n")
                    elif matchs_ville != None:
                        print("Voici tout les matchs qui ce sont dérouler dans la ville de "+str(ville)+" : "+str(matchs_ville))
                        non_réussi = False
        # Option 3
        if rep == 3:
            if liste_matchs == None:
                ...
                non_réussi = True
                while non_réussi:
                    ville = input("De quelle compétition veux tu le nombre de buts moyen ? ")
                    nb_moyen_buts = histoire2foot.nombre_moyen_buts(liste_matchs, ville)
                    if matchs_ville != None:
                        print("Il y a eu une moyenne de "+str(nb_moyen_buts)+" dans cette compétition")
                    else:
                        quitter = input("Ce nom de compétition n'a pas été trouvée. Veux tu quitter ? (o pour quitter sinon entrée)" )
                        if quitter == "o":
                            non_réussi = False
        # Option 4
        if rep == 4:
        ...
        if rep == 5:
            ...
            while (non_réussi1 or non_réussi2) and non_réussi:
                while non_réussi1 and non_réussi:
                    try:
                        fic1 = histoire2foot.charger_matchs("./"+input("Quel est le premier fichier que tu veux charger ? \n"))
                ...
                while non_réussi2 and non_réussi:
                    try:
                        fic2 = histoire2foot.charger_matchs("./"+input("Quel est le deuxieme fichier que tu veux charger ? \n"))
        ...
        if rep == 6:
            if liste_matchs == None:
                ...
                stats = histoire2foot.resultats_equipe(liste_matchs, input("De quelle équipe veux tu les statistiques ?"))
                if stats == (0,0,0):
                    print("Cette équipe n'a jamais joué")
                else:
                    print("Cette équipe a joué "+str(stats[0]+stats[1]+stats[2])+" matchs dont : "+str(stats[0])+" victoires, "+str(stats[1])+" nuls et "+str(stats[2])+" défaites")
...
# programme_principal()
# print(histoire2foot.resultats_equipe(histoire2foot.charger_matchs("histoire1.csv"),"Scotland"))